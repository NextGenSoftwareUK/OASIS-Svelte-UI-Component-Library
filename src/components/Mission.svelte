<script lang="ts">
  ...
  function accept(m: Mission) { m.status = 'active'; missions = [...missions] }
  function complete(m: Mission) { m.status = 'complete'; missions = [...missions] }
</script>
<div class="mis-shell">
    ...
    {#each missions as m (m.id)}
      <div class="mis-card mis-card--{m.status}">
        ...
        <div class="mis-right">
          ...
          {#if m.status === 'available'}<button class="mis-btn" on:click={() => accept(m)}>Accept</button>{/if}
          {#if m.status === 'active'}<button class="mis-btn mis-btn--complete" on:click={() => complete(m)}>Complete</button>{/if}
        </div>
      </div>
    {/each}
  </div>
