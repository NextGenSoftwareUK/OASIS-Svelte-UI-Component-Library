<script lang="ts">
  ...
  function toggle(p: Provider) { p.connected = !p.connected; providers = [...providers] }
</script>
    ...
    {#each providers as p (p.id)}
      <div class="prov-card" class:prov-card--connected={p.connected}>
        ...
        <div class="prov-actions">
          ...
          <button class="prov-btn" class:prov-btn--disconnect={p.connected} on:click={() => toggle(p)}>{p.connected ? 'Disconnect' : 'Connect'}</button>
        </div>
      </div>
    {/each}
  </div>
