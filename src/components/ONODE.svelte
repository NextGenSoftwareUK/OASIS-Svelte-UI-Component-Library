...
<div class="nd-shell">
  ...
  <div class="nd-status-row">
    ...
    <button class="nd-btn" class:nd-btn--stop={running} on:click={() => running = !running}>{running ? 'Stop Node' : 'Start Node'}</button>
  </div>
  {#if running}
    <div class="nd-metrics">
      {#each metrics as m}<div class="nd-metric"><div class="nd-metric-val">{m.value}</div><div class="nd-metric-label">{m.label}</div></div>{/each}
    </div>
    <div class="nd-logs-header">Node Logs</div>
    <div class="nd-logs">
      {#each logs as l (l.time)}
