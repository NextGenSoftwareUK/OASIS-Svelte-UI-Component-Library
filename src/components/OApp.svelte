<script lang="ts">
  ...
  $: filtered = oapps.filter(a => a.name.toLowerCase().includes(search.toLowerCase()) || a.description.toLowerCase().includes(search.toLowerCase()))
  function toggle(a: OApp) { a.launched = !a.launched; oapps = [...oapps] }
</script>
<div class="oapp-shell">
  ...
  <input class="oasis-input" type="text" bind:value={search} placeholder="Search OApps…" />
  <div class="oapp-grid">
    {#each filtered as a (a.id)}
      <div class="oapp-card">
        ...
        <div class="oapp-footer">
          ...
          <button class="oapp-btn" class:oapp-btn--launched={a.launched} on:click={() => toggle(a)}>{a.launched ? 'Quit' : 'Launch'}</button>
        </div>
      </div>
    {/each}
    {#if filtered.length === 0}<div class="oapp-empty">No OApps found.</div>{/if}
  </div>
</div>
<style>
